...
<body>
	<article class="col-7 fl">
		<div class="u-r-cont">
			<section>
				<div>
					<span class="fr"><a href="/front/jump" title=""
						class="comm-btn c-btn-6"
						style="font-size: 16px; height: 22px; line-height: 22px; padding: 0 20px;">去提问</a></span>
					<section class="c-infor-tabTitle c-tab-title cnew-tab-title">
						<a href="${ctx }javascript: void(0)" title="Wo的问答"
							style="cursor: default;">Wo的问答</a> <a
							href="${ctx }/uc/myquestions/list" title="Wo的提问">Wo的提问</a> <a
							href="${ctx }/uc/myrepquestions/list" title="Wo的回答"
							class="current">Wo的回答</a>
					</section>
				</div>
				<div class="mt40">
					<c:if test="${empty comments }">
						...
						<section class="no-data-wrap">
							<em class="icon30 no-data-ico">&nbsp;</em> <span
								class="c-666 fsize14 ml10 vam">亲，您还没有回复问答,赶快去回答吧！</span>
						</section>
						<!-- /无数据提示 结束-->
					</c:if>
					<c:if test="${not empty comments }">
						<div class="u-question-wrap">
							<section class="q-all-list">
								<ul>
									<c:forEach items="${comments }" var="question">
										<li>
											<aside class="q-head-pic">
												<c:choose>
													<c:when test="${not empty question.edu_user.pic_img }">
														<img src="<%=staticImage %>${question.edu_user.pic_img }"
															alt="">
													</c:when>
													<c:otherwise>
											...
											<section class="q-txt-box">
													...
													<c:if test="${not empty question.questions.content  }">
														<c:if test="${question.questions.status==0 }">
															<span class="fsize12 c-999 vam"> <tt
																	...
																	items="${question.questions.content }"
																	var="questionsComment">
																	<c:out value="${question.questions.content }"></c:out>
																</c:forEach>
															</span>
															<!-- 有回答时显示最新一条的回答内容 -->
														</c:if>
												...
												<div class="mt15">
													<span class="c-ccc fl vam">时间：<fmt:formatDate
															value="${question.add_time }" type="date"
															pattern="yyyy-MM-dd hh:mm:ss" /></span>
													<section class="fl ml20 pt10">
														<div class="taglist clearfix">
															<c:forEach items="${question.questions.questions_tags}"
																var="questionsTag">
																<a title="${questionsTag.questions_tag_name }"
																	data-id="${questionsTag.questions_tag_id}"
																	href="#<%-- ${ctx}/questions/list?questions.questionsTagId=${questionsTag.questionsTagId } --%>">${questionsTag.questions_tag_name }</a>
															</c:forEach>
														</div>
													</section>
													<div class="clear"></div>
					...
					<!-- 公共分页 结束 -->
					<form action="${ctx}/uc/myrepquestions/list" id="searchForm"
						method="post">
