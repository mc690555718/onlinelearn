            ...
            '<div id="d-s-head-cont" class="lrWrap">'+
                ...
                '<section class="undis e-login-ele">'+
                    ...
                    '<div>'+
                        '<ol class="e-login-options">'+
                            '<li>'+
                                '<input id="u-email-reg" type="text" placeholder="请输入登录邮箱"  name="" value="" onblur="register()" onkeyup="$(this).next().html(\'\');">'+
                                '<p class="lr-tip-wrap"></p>'+
                            '</li>'+
                            '<li>'+
	...
	$.ajax({
		...
		success:function(result){
			if(result.success==false){
				...
				if(type==1){
					window.location.href="/frontuser/front";
				}else{
...
function dialogRegister() {
	...
	var oldPaaword=$('#u-password-reg').val();
	var newPassword=$('#u-passwordre-reg').val();
	...
	if(oldPaaword!=newPassword){
		$("#u-passwordre-reg").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>两次密码不一致！</span>');
		return false;
	}
	$.ajax({
		url :"/frontuser/isnsertUser",
		data : {"email":$("#u-email-reg").val(),"password":$("#u-password-reg").val(),
				"mobile":$("#u-mobile-reg").val()
				},
		type : "post",
		dataType :"json",
		cache : false,
...
function register(){
	var emailVal=$("#u-email-reg").val();
	$.post( "/frontuser/register",{"email":emailVal},function(msg){
		if(msg.email!=emailVal){
		}else{
			$("#u-email-reg").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>此用用户已存在！</span>');
			return false;
		}
	});
}
...
function getCourseLearnedUser(courseId){
	$.ajax({
		type : "post",
		dataType : "json",
		url:"/Course/ajax/courseLearnedUser?courseId="+courseId,
		cache : true,
		async : false,
		success : function(result) {
				...
				if(resultList.length!=0){
					var useImg="";
	 				...
	 				for(var i=0;i<resultList.length;i++){
	 					useImg=resultList[i].user.pic_img;
	 					if(useImg==null || $.trim(useImg)==''){
	 						useImg ='/static/inxweb/img/avatar-boy.gif';
	 					}else{
	 						useImg =useImg;
	 					}
	 					userShowName=resultList[i].userShowName;
	 					if(userShowName==null || $.trim(userShowName)==''){
	 						userShowName = resultList[i].user.show_name;
	 					}
